import type { DocumentContext, ChunkMetadata, TranscriptSegment } from '@tab-zen/shared';
...
export function chunkTranscript(
  ...
  for (const segment of segments) {
    ...
    if (segmentStart - windowStart >= windowSeconds && currentChunk.length > 0) {
      chunks.push({
        text: currentChunk.map((s) => s.text).join(' '),
        position,
        metadata: {
          timestampStart: formatTimestamp(windowStart),
          timestampEnd: formatTimestamp(currentChunk[currentChunk.length - 1].startMs / 1000),
        },
      });
      position++;
