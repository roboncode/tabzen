...
export default function SettingsPanel(props: SettingsPanelProps) {
  ...
  const [aiTestResult, setAiTestResult] = createSignal<{
    ok: boolean;
    message: string;
  } | null>(null);
  ...
  const sidebarNav = (onItemClick?: () => void) => (
    <>
      ...
      <div class="h-16 flex items-center px-5 flex-shrink-0">
        <button
          onClick={() => navigate("/")}
          class="text-sm font-bold text-foreground hover:text-foreground/80 transition-colors"
        >
          Tab Zen
        </button>
      </div>
      <div class="mx-5 border-b-3 border-muted-foreground/10 flex-shrink-0" />

      {/* Nav items — scrollable */}
      <nav class="flex flex-col gap-0.5 px-5 pt-6 pb-4 flex-1 min-h-0 overflow-y-auto scrollbar-hide">
        <For each={navGroups}>
          {(group, groupIndex) => (
            <>
              <Show when={groupIndex() > 0}>
                <div class="my-3 border-b border-muted-foreground/10 flex-shrink-0" />
              </Show>
              <For each={group.items}>
                {(item) => (
                  <button
                    class={`w-full flex items-center px-2.5 py-1.5 rounded-lg text-sm transition-colors flex-shrink-0 ${
      ...
      <div class="px-5 pb-5 pt-4 flex-shrink-0 text-center">
        <span class="text-xs text-muted-foreground/40">v{browser.runtime.getManifest().version}</span>
      </div>
    </>
  );
  ...
  return (
    <div class="flex h-full bg-background text-foreground @container">
      ...
      <div class="hidden @[768px]:flex w-72 flex-shrink-0 h-full bg-[#161618] overflow-y-auto scrollbar-hide flex-col">
        {sidebarNav()}
      </div>
      ...
      <div class="flex-1 min-w-0 flex flex-col h-full">
        ...
        <div class="flex items-center gap-2 px-4 py-4 bg-background border-b-3 border-[#161618] flex-shrink-0">
          ...
          <button
            class="w-8 h-8 rounded-md flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors @[768px]:hidden flex-shrink-0"
          ...
          <span class="text-sm font-medium text-foreground flex-shrink-0">Settings</span>
              ...
              <div class="max-w-2xl mx-auto p-8 space-y-4">
                  ...
                  <div class="space-y-6">
                    <div>
                      ...
                      <input
                        ...
                        value={s().sourceLabel}
                        onChange={(e) =>
                          save({ sourceLabel: e.currentTarget.value })
                        }
                      />
                      ...
                      <div class="flex items-center justify-between mb-3">
                        <div>
                          <p class="text-sm text-foreground">
                            Capture button on pages
                          </p>
                          <p class="text-xs text-muted-foreground mt-0.5">
                            Show a floating save button on every web page
                          </p>
                        </div>
                        <button
                          ...
                          onClick={() =>
                            save({ notchEnabled: !s().notchEnabled })
                          }
                        >
                          <div
                            class={`w-4 h-4 rounded-full bg-white mx-1 transition-transform ${
                              s().notchEnabled
                                ? "translate-x-4"
                                : "translate-x-0"
                            }`}
                  ...
                  <div class="space-y-4">
                    <div>
                      ...
                      <select
                        ...
                        value={s().aiModel}
                        onChange={(e) =>
                          save({ aiModel: e.currentTarget.value })
                        }
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        value={s().chatModel}
                        onChange={(e) =>
                          save({ chatModel: e.currentTarget.value })
                        }
                      >
                        <option value="anthropic/claude-sonnet-4">
                          Claude Sonnet 4
                        </option>
                        <option value="anthropic/claude-haiku-4">
                          Claude Haiku 4
                        </option>
                ...
                <Show when={activeSection() === "api-keys"}>
                  <div class="space-y-5">
                    <div>
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (response.ok) {
                                ...
                                setAiTestResult({
                                  ok: false,
                                  message: `Invalid key (${response.status})`,
                                });
                              }
                            } catch (e) {
                              setAiTestResult({
                      ...
                      <Show when={aiTestResult()}>
                        {(result) => (
                          <div class="flex items-center gap-2 mt-2">
                            <div
                              class={`w-2 h-2 rounded-full ${result().ok ? "bg-green-500" : "bg-red-500"}`}
                            />
                            <span
                              class={`text-xs ${result().ok ? "text-green-400" : "text-muted-foreground"}`}
                            >
                              {result().message}
                            </span>
                          </div>
                      ...
                      <p class="text-xs text-muted-foreground mt-1.5">
                        Required for AI features. Without a key, pages are
                        grouped by domain.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="gsk_..."
                        onInput={(e) =>
                          save({ groqApiKey: e.currentTarget.value.trim() })
                        }
                      />
                      <p class="text-xs text-muted-foreground mt-1.5">
                        Optional — enables voice input in chat via Whisper
                        transcription
                      </p>
                    </div>
                  </div>
                ...
                <Show when={activeSection() === "features"}>
                  <div class="space-y-4">
                    <div class="flex items-center justify-between">
                      <div>
                        <p class="text-sm text-foreground">
                          Auto-tag pages on capture
                        </p>
                        <p class="text-xs text-muted-foreground mt-0.5">
                          Generate tags when pages are saved
                        </p>
                      </div>
                      <button
                        class={`w-10 h-6 rounded-full transition-colors ${
                    ...
                    <div class="flex items-center justify-between">
                      <div>
                        <p class="text-sm text-foreground">
                          Auto-generate chapters for videos
                        </p>
                        <p class="text-xs text-muted-foreground mt-0.5">
                          Create chapter headings from video transcripts
                        </p>
                      </div>
                      <button
                        class={`w-10 h-6 rounded-full transition-colors ${
                        ...
                        onClick={() =>
                          save({ autoChapters: !s().autoChapters })
                        }
                        disabled={!s().openRouterApiKey}
                      >
                        <div
                    ...
                    <div class="flex items-center justify-between">
                      <div>
                        ...
                        <p class="text-xs text-muted-foreground mt-0.5">
                          Compress long documents to save tokens in chat (50-70%
                          savings)
                        </p>
                      </div>
                      <button
                        ...
                        onClick={() =>
                          save({ chatCompression: !s().chatCompression })
                        }
                      >
                        <div
                          class={`w-4 h-4 rounded-full bg-white mx-1 transition-transform ${
                ...
                <Show when={activeSection() === "data"}>
                  <div class="space-y-6">
                    {/* Export */}
                    <div class="space-y-2">
                      <p class="text-xs font-semibold uppercase tracking-wider text-muted-foreground/50 px-1">
                        Export
                      </p>
                      <button
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p class="text-xs text-muted-foreground mt-0.5">
                            HTML bookmark file compatible with any browser
                          </p>
                        </div>
                      </button>
                    </div>
                    ...
                    <div class="space-y-2">
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p class="text-xs text-muted-foreground mt-0.5">
                            Configure hotkeys for quick capture and navigation
                          </p>
                        </div>
                      </button>
                    </div>
                    ...
                    {/* Danger Zone */}
                    <div class="space-y-2">
                      <p class="text-xs font-semibold uppercase tracking-wider text-red-400/50 px-1">
                        Danger Zone
                      </p>
                      <button
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p class="text-xs text-muted-foreground mt-0.5">
                            Remove pages from this profile or clear everything
                          </p>
                        </div>
                      </button>
                    </div>
                    ...
                    <div class="space-y-3">
                      ...
                      >
                        ...
                        <p class="text-xs text-muted-foreground mt-1">
                          Only removes pages captured from "
                          {settings()?.sourceLabel}". Data from other devices is
                          kept.
                        </p>
                      </button>
                      <button
                      ...
                      >
                        ...
                        <p class="text-xs text-muted-foreground mt-1">
                          Removes all pages, groups, and captures from every
