...
export class ContentService {
  ...
  async storeTranscript(syncToken: string, pageId: string, segments: TranscriptSegment[]): Promise<string> {
    const key = `${syncToken}/transcripts/${pageId}.json`;
    await this.r2.put(key, JSON.stringify(segments), {
      httpMetadata: { contentType: "application/json" },
    });
    return `transcripts/${pageId}`;
  }

  async getTranscript(syncToken: string, pageId: string): Promise<TranscriptSegment[] | null> {
    const key = `${syncToken}/transcripts/${pageId}.json`;
